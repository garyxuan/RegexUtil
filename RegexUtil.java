...
public class RegexUtil {
	...
	/**
	 * 正则表达式的特殊字符，需要进行转义处理 java \\代表一个反斜杠 其后的字符具有特殊意义 \\\\ 表示一个普通的反斜杠 不影响其后的字符
	 * 这些字符前都要加上 \\才能正确表达一个正则串
	 */
	private static final String specialChar = ".+*\\$^?{}()[]\\|";

	/**
	 * 匹配汉字 汉字的unicode值范围是\u4e00-\u9fa5
	 */
	...
	public static void main(String[] args) {
		...
		emaiRegex1.append(RegexUtil.letterAndNumAndUnderLine).repeatOneOrMore().append("@")
				.append(new RegexUtil(RegexUtil.letterAndNumAndUnderLine).or(".").or("-").repeatOneOrMore()).append(".")
				.append(new RegexUtil(RegexUtil.lowLetter).repeatOneOrMore());
